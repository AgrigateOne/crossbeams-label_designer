...
const LabelDesigner = (function LabelDesigner() { // eslint-disable-line max-classes-per-file, no-unused-vars
  ...
  class LdVariable extends LdShape {
    generate(opts = {}) {
      ...
      const varAttrs = {
        ...
        varFormat: optAttrs.varFormat || null,
      };
  ...
  const init = (labelConfig) => {
    ...
    ldState.variableUI = {
      ...
      labelVarFormatWrapper: document.querySelector('#label_var_format_wrapper'),
      labelVarFormat: document.querySelector('#label_var_format'),
    };
  ...
  const saveVariableSettings = (variableTypeValue) => {
    ...
    const varAttrs = {
      ...
      varFormat: ldState.variableUI.labelVarFormat.value,
    };
  ...
  const varChange = (value) => {
    if (value === 'Static Barcode') {
      ldState.variableUI.staticInput.style.display = 'block';
    ...
    // Display or hide variable formats if applicable:
    const fmts = ldState.labelConfig.formatRules[value];
    const select = ldState.variableUI.labelVarFormat.selectr;
    if (fmts) {
      const newItems = [];
      ldState.variableUI.labelVarFormatWrapper.hidden = false;
      fmts.forEach(fmt => {
        newItems.push({
          value: fmt,
          label: ldState.labelConfig.formatDescriptions[fmt],
        });
      });
      ...
      if (curr.varFormat) {
        select.setChoiceByValue(curr.varFormat);
    } else {
        select.setChoiceByValue(newItems[0].value);
      }
    } else {
      ldState.variableUI.labelVarFormatWrapper.hidden = true;
      select.removeActiveItems();
    }
  };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const holdSel = new Choices(ldState.variableUI.variableSelect, {
      ...
      classNames: {
        ...
        groupHeading: ['choices_tw_heading'],
      },
      shouldSort: false,
      searchFields: ['label'],
      // allowHTML: true,
    ...
    // Make the variable format select a Choices dropdown
    const holdFmtSel = new Choices(ldState.variableUI.labelVarFormat, {
      searchEnabled: true,
      searchResultLimit: 100,
      itemSelectText: '',
      classNames: {
    ...
    // Store a reference on the DOM node.
    ldState.variableUI.labelVarFormat.selectr = holdFmtSel;
    ...
    shapes.forEach((shape) => {
      ...
      if (shape.attrs.name === 'VariableBox') {
        ...
        varAttrs = {
          ...
          varFormat: shape.attrs.varFormat || null,
        };
